...
import epp_pso.tribes_new.EssaimTribes_new;
 ...
 */
public class EvolutionPPnew extends EvolutionPP
{
	/*---------*/
	/* Données */
	/*---------*/
	...
	private double[] A;						// Enregistre la mailleure projection de la simulation en cours
	...
	private ArrayList<OrdonnerPP> liste;	// Liste pour récupérer les I et les vecteurs associés dans le fichier résultat des simulations
	...
	private	EssaimTribes_new	tribe;

	private int next_adaptation;
	 ...
	 */
	public EvolutionPPnew (ParametrePP p)
		{
		super(p);
		this.parametre = p;
	...
	public void demarre ()
		...
		if (this.cpt_iteration == 0 && this.cpt_simulation == 0)
			...
			Matrice X = new Matrice(path, this.cpt_jeu_de_donnees);

			/*----- Matrice de travail 'matrice' -----*/
			long start = System.currentTimeMillis();
			System.out.print("Transformation des données ... ");

			Matrice matrice;
			if (this.parametre.sphere)
				{
				System.out.print("spherique");
				...
				if (this.parametre.ficherDeDonnees.equals("geno_transpose.txt"))
					matrice = new Matrice(this.parametre.rep_donnees + File.separatorChar + "geno_transpose_sph.txt");
				else
					matrice = X.spherique();
				}
			else
				{
				System.out.print("centrees reduites");
			...
			System.out.println(" ... terminée (en " + StringUtils.convertHMSms(stop-start) + ")");

			matrice.setNom(this.parametre.ficherDeDonnees);

			/*----- Instanciation de l'indice -----*/
			if (this.parametre.nomIndice == ParametrePP.KURTOSIS_MAX)			this.indice = new KurtosisMax(matrice);
			else if (this.parametre.nomIndice == ParametrePP.KURTOSIS_MIN)		this.indice = new KurtosisMin(matrice);
			else if (this.parametre.nomIndice == ParametrePP.FRIEDMAN)			this.indice = new Friedman(matrice);
			else if (this.parametre.nomIndice == ParametrePP.FRIEDMAN_TUKEY)	this.indice = new FriedmanTukey(matrice);
			else if (this.parametre.nomIndice == ParametrePP.DISCRIMINANT)		this.indice = new Discriminant(matrice);
			else if (this.parametre.nomIndice == ParametrePP.INDICE_4)			this.indice = new Indice4(matrice);
			else if (this.parametre.nomIndice == ParametrePP.STAHEL_DONOHO)		this.indice = new StahelDonoho(matrice);
			...
			if (this.cpt_jeu_de_donnees == 0)
				 ...
				 *  - le nombre de jeux de données (matrices d'observations) à traiter.
				...
				sb.append("<Transformation_donnees>   ").append(this.parametre.sphere ? "Spherique" : "Centree_reduite").append('\n');
				...
				sb.append("<Nombre_d_individus>       ").append(this.parametre.methode == ParametrePP.TRIBES ? "?" : this.parametre.getNbIndividus()).append('\n');
				if (this.parametre.methode == ParametrePP.GA)		sb.append(this.parametre.pag); else
				if (this.parametre.methode == ParametrePP.PSO)		sb.append(this.parametre.ppso); else
				if (this.parametre.methode == ParametrePP.TRIBES)	sb.append(this.parametre.ptribe);

				/*
				 * On positionne une balise à la place du nombre initial de
				 * lancements qui sera remplacée en fin d'écriture du fichier.
			...
			/*----- Sortie écran -----*/
			System.out.println("Jeu de donnees " + this.cpt_jeu_de_donnees);
		...
		if (this.parametre.methode.equals(ParametrePP.TRIBES))
			{
			/*----- Essaim -----*/
			this.tribe = new EssaimTribes_new(this.indice);

			this.next_adaptation = 1;
...
//				this.tribe.creationTribu(this.indice);
		...
		if (this.parametre.methode == ParametrePP.MULTISTART)
			{
...
//		System.out.print(this.cpt_iteration + " ");
		...
		/*----- Itération -----*/
		if (this.parametre.methode == ParametrePP.GA)
			{
			/*----- GA -----*/
			this.pop.generation();
		...
		if (this.parametre.methode == ParametrePP.PSO)
			{
			/*----- PSO -----*/
		...
		if (this.parametre.methode == ParametrePP.TRIBES)
			{
			/*----- TRIBES -----*/
//			this.tribe.updateBest();
//			this.tribe.updatePosition(false, 0);
...
//				this.tribe.adaptation(this.indice);
			...
			System.out.print("Tribu : " + this.tribe.getNbTribus());

			System.out.print("(");
			for (int mm=0; mm<this.tribe.getNbTribus(); mm++)
				System.out.print(this.tribe.getTribu(mm).size() +
//								 " " + this.tribe.swarm.get(mm).isGood() +
//								 " " + GuiUtils.DECIMAL_3.format(this.tribe.swarm.get(mm).getBestParticleOfTribe().meilleurI) + " " +
								 " ");
			System.out.print(")");
			
			System.out.println(" particules : " + this.tribe.getNbParticules());

			if (this.cpt_iteration == this.next_adaptation)
				{
				this.tribe.adaptation(this.indice);

				System.out.print("Adaptation " + next_adaptation + " --> ");
				this.next_adaptation += this.tribe.nbLiens()/2+1;
				System.out.println(next_adaptation);
				}
	...
	public void termine ()
		...
		if (this.getEtatPrecedent() != Evolution.EXECUTABLE)
			...
			/*----- Affichage graphique -----*/
			if (this.frRecherche != null && this.frRecherche.isVisuLancement()) this.affProjection(this.A);

			/*----- Statisstiques : enregistrement du meilleur I et de la meilleure projection de la simulation -----*/
			...
			/*----- Sortie écran -----*/
			System.out.print("... terminé (I " + GuiUtils.DECIMAL_9.format(this.I) + " en " + GuiUtils.DECIMAL_3.format(this.getElapsedTime()/1000.0) + "s)");
...
//			if (this.parametre.methode == ParametrePP.TRIBES) System.out.print(" Nb de tribu : " + this.tribe.getNbTribus());
		...
		/**
		 * On regarde si l'utilisateur n'a pas stoppé et si toutes les
		 ...
		 */
			...
			/*----- Affichage graphique -----*/
			if (this.frRecherche != null && this.frRecherche.isVisuFin()) this.affProjection(this.meilleureProjectionA);
			...
			/**
			 * On regarde si l'utilisateur n'a pas stoppé et si tous les jeux
			 * de données sont été traités.
			 */
			if (!this.isStopUser() && (this.cpt_jeu_de_donnees+1) < this.nb_de_jeu_de_donnees)
				...
				if (this.parametre.nomIndice == ParametrePP.FRIEDMAN)				nomFichierSortie += "F_";
				else if (this.parametre.nomIndice == ParametrePP.FRIEDMAN_TUKEY)	nomFichierSortie += "FT_";
				else if (this.parametre.nomIndice == ParametrePP.KURTOSIS_MAX)		nomFichierSortie += "KurtoMax_";
				else if (this.parametre.nomIndice == ParametrePP.KURTOSIS_MIN)		nomFichierSortie += "KurtoMin_";
				else if (this.parametre.nomIndice == ParametrePP.DISCRIMINANT)		nomFichierSortie += "I3_";
				else if (this.parametre.nomIndice == ParametrePP.INDICE_4)			nomFichierSortie += "I4_";
				else if (this.parametre.nomIndice == ParametrePP.STAHEL_DONOHO)		nomFichierSortie += "SD_";

				if (this.parametre.methode == ParametrePP.GA)				nomFichierSortie += "GA_";
				else if	(this.parametre.methode == ParametrePP.PSO)			nomFichierSortie += "PSO_";
				else if	(this.parametre.methode == ParametrePP.TRIBES)		nomFichierSortie += "TRIBE_";
				else if (this.parametre.methode == ParametrePP.MULTISTART)	nomFichierSortie += "MULTISTART_";
				...
				if (this.frRecherche != null)
					...
					System.out.println("Fichier résultat : " + nomFichierSortie);
					System.out.println("----------");
...
//					System.exit(0);
	...
	public boolean conditionEvolution ()
		{
		if (this.cpt_iteration++ < this.parametre.getNbIterations())
			return true;
		else
			return false;
		}
